...
In the end-user-created ticket example below, `Karen` (best reached at `Karen@Karen.com` ) is submitting a ticket regarding her trouble to access her account .
...
In the example below, the admin first clicks on the greyed out `— Unassigned —` text that's located to the right of **Assigned To**. Then, the admin designates `Finn John` as the **Assignee**.

<img src="1/Screenshot 2026-09-19 001415.png" alt="Assign Agent to Ticket">

After the agent has been assigned to this particular ticket, the admin sets the **Priority** to `High`, the **Department** to `Support`, and the **SLA Plan** to `SEV-B`.
...
2. Then, osTicket will take the Agent back to the Tickets tab. The Agent will then see a confirmation that the reply was posted successfully. As pictured below, ticket `#325542` by Karen is no longer showing in the **Open** tickets section. Additionally, there is a visual confirmation banner at the top that says `Ticket #671497: Reply posted successfully`.
